...
namespace boost
{
    namespace range_detail
    {
        namespace view
        {
            ...
            {
                // Non optimized version of std::advance that check for its
                // bound.
                for ( ; it != end && n > 0; --n )
                {
                    ++it;
            ...
            template < class Iter >
            class chunk_iterator
                : public chunk_iterator_base_gen< Iter >::base_t
            {
                typedef
                    typename chunk_iterator_base_gen< Iter >::base_t super_t;
    ...
    namespace view
    {
        using range_detail::view::chunk_range;
    }
    namespace adaptors
    {
        namespace view
        {
            using boost::range_detail::view::chunk_range;

            namespace
            {
                const range_detail::forwarder<
                    range_detail::view::chunk_holder > chunked =
                    range_detail::forwarder<
                        range_detail::view::chunk_holder >();
            }
            ...
            template < class ForwardRng >
            inline chunk_range< const ForwardRng > chunk(
                const ForwardRng& rng,
                std::size_t chunk_size )
            {
                BOOST_RANGE_CONCEPT_ASSERT(
